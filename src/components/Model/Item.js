...
import { deleteModelAsync, setSelected, updateModelAsync } from '../../redux/modelSlice';
...
const CategoryItem = ({item}) => {
    ...
    const selected = useSelector((state) => state.model.selected);

    const handleDeleteClick = (e) => {

        e.preventDefault();
        dispatch(setSelected([]));
        dispatch(deleteModelAsync({
            id:item.id
        }))
    ...
    const handleUpdateClick = (event) => {
        ...
        if(selected.title!=''){
            ...
            dispatch(updateModelAsync(selected))
        ...
        (
            <li className="flex py-1 border-b border-header-border-color border-dashed justify-between">
                {selected.id === item.id ? (
                    <>
                        <div className='w-[85%]'>
                            <input type="text" id={`model_${item.id}`} className='w-full outline-0 border border-header-border-color rounded-full px-3 h-8' onChange={handleChange} value={selected.title} />
                        </div>
